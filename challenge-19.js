(function(){
    ...
    */
    ...
    function hasClass(markup, cssClass){
            var regExp = new RegExp('class=[\\W]+([\\w\\s]+)', 'mg');
            var regExpIn = new RegExp('(^'+cssClass+'\\n)', 'mg');
            var aux = '';
            function findClass(regex, classe) {
                    aux += classe + '\n';
                    return classe;
            }
            markup.replace(regExp, findClass);
            var aux2 = aux.match(regExpIn);
            if(aux2 !== null){
                return true;
            }
            return false;
